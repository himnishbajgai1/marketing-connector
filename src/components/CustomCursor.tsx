...
export function CustomCursor() {
  ...
  return (
    <>
      ...
      >
        <div className="h-4 w-4 rounded-full border-2 border-white transition-all duration-200" />
      </div>
      <div
        ref={innerRef}
        className="pointer-events-none fixed left-0 top-0 z-[100] mix-blend-difference will-change-transform hidden md:block"
        style={{ contain: "layout style paint" }}
      >
        <div className="h-2 w-2 rounded-full bg-white transition-all duration-200" />
      </div>
    </>
  );
}
